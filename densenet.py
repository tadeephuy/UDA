...
class _DenseLayer(nn.Sequential):
    def __init__(self, num_input_features, growth_rate, bn_size, drop_rate, se):
        super(_DenseLayer, self).__init__()
        if se:
            # Add SELayer at here, like SE-PRE block in original paper illustrates
            self.add_module("selayer", SEModule(channels=num_input_features)),
...
class _DenseBlock(nn.Sequential):
    def __init__(self, num_layers, num_input_features, bn_size, growth_rate, drop_rate, se):
        super(_DenseBlock, self).__init__()
        for i in range(num_layers):
            layer = _DenseLayer(num_input_features + i * growth_rate, growth_rate, bn_size, drop_rate, se)
...
class SEDenseNet(nn.Module):
    ...
    def __init__(self, growth_rate=32, block_config=(6, 12, 24, 16),
                 num_init_features=64, bn_size=4, drop_rate=0, num_classes=1000, num_channels=3, se=True):
        ...
        for i, num_layers in enumerate(block_config):
            ...
            block = _DenseBlock(num_layers=num_layers, num_input_features=num_features,
                                bn_size=bn_size, growth_rate=growth_rate, drop_rate=drop_rate, se=se)
            ...
            if i != len(block_config) - 1:
                if se:
                    ...
                    self.features.add_module("SELayer_%db" % (i + 1), SEModule(channels=num_features))
        ...
        if se:
            # Add SELayer
            self.features.add_module("SELayer_0b", SEModule(channels=num_features))
    ...
    def forward(self, x):
        ...
        out = F.avg_pool2d(out, kernel_size=features.shape[2], stride=8)
...
if __name__ == '__main__':
    inp = torch.randn([8, 1, 320, 390])

    model = se_densenet201(pretrained=False, num_channels=1, num_classes=5)
